            ...
            req = https.get(this.parsedOpts()["source_file"], function(res) {
              ...
              res.on("end", function(e) {
                if ([200,304].indexOf(res.statusCode) != -1) {
                  ...
                  job.exitHandler(-1, "bad status code [" + res.statusCode + "] trying to download (" + job.parsedOpts()["source_file"] + ").");
                }
              });
            }).on("error", function(e) {
              job.exitHandler(-1, "unable to download file (" + job.parsedOpts()["source_file"] + ").");
            });
    ...
    probe: function() {
      ...
      the_process.on('exit', function(code) {
        ...
        if (outputBuffer != undefined) {
          probeDataSane = outputBuffer.toString('utf-8').split('\n').filter(
            function(val) {
