...
var Job = JobUtils.getDatabase().define('Job', {
  ...
  instanceMethods: {
    ...
    spawn: function() {
      var job = this;
      if (this.hasInput && this.hasOutput && !this.the_process) {
        ...
        var transcodeArgs = this.parsedOpts()['encoder_options'].replace(/\s+/g, " ").split(' ');
        transcodeArgs.unshift('-i', this.sourceFilePath);
        ...
        transcodeArgs.push(this.destinationFilePath);
        ...
        if (this.inputType == "http") {
          ...
          if (this.parsedOpts()["source_file"].match(/^https/i)) {
            ...
            req = https.get(this.parsedOpts()["source_file"], function(res) {
              ...
              res.on("end", function(e) { job.transcode(transcodeArgs) });
            }).on("error", function(e) {
    ...
    transcode: function(transcodeArgs) {
      var job = this;
      ...
      the_process.on('exit', function(code) { job.finalize(code, job.destinationFilePath); });
    ...
    finalize: function(code, tmpFile) {
      ...
      if (code == 0) {
        switch(this.outputType) {
          case "s3":
            var destination = job.parsedOpts()['destination_file'].match(/^(s3:\/\/)([a-z0-9\.\_\-]+)\/(.+)$/)
            ...
            try {
              ...
              fs.readFile(tmpFile, function (err, data) {
                ...
                var s3         = new aws.S3();
                var filebody   = new Buffer(data, 'binary').toString('base64');
